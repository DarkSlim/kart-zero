...
func _ready(): #{
	...
	#set_process(true); set_fixed_process(false);
	set_process(false); set_fixed_process(true);
...
#func _process(delta): #{ # UGLYHACK FIXME TODO
	delta = min( delta, 1.0 / 30.0 ); # to prevent physics simulation droping below tolerable frequency
	...
	if is_computer :
		computer_logic(delta) # TODO : call it less often ?
		#pass
